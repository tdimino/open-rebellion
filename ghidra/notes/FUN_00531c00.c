
int __thiscall FUN_00531c00(void *this,void *param_1)

{
  bool bVar1;
  undefined3 extraout_var;
  int iVar2;
  int iVar3;
  undefined4 local_4;
  
  bVar1 = FUN_0053a010((int)this);
  local_4 = CONCAT31(extraout_var,bVar1);
  if (local_4 != 0) {
    iVar2 = FUN_00539fd0(this,0);
    iVar3 = FUN_005404d0(this,*(uint *)((int)param_1 + 8));
    if (iVar3 != 0) {
      iVar3 = *(int *)((int)this + 0x84);
      if (iVar3 != *(int *)(iVar2 + 0x84)) {
        *(int *)((int)this + 0x84) = *(int *)(iVar2 + 0x84);
        iVar2 = FUN_004fd3d0();
        if (iVar2 != 0) {
          iVar2 = FUN_004fd600(param_1,*(uint *)((int)this + 0x24) >> 4 & 3);
          if (iVar2 != 0) {
            local_4 = (**(code **)(*(int *)this + 0x230))
                                (iVar3,*(undefined4 *)((int)this + 0x84),param_1);
          }
        }
      }
    }
  }
  return local_4;
}

