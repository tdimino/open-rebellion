...
- **Combat**: space (fleet engagement, 4-arc weapons, shields) and ground (troops + orbital bombardment). **RE COMPLETE** — 5,127 functions decompiled from REBEXE.EXE. Bombardment formula decoded (Euclidean distance / GNPRTB). Space combat 7-phase pipeline mapped. 71 combat GNPRTB parameters identified. See `ghidra/notes/` for full docs.
