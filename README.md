...
If you spent hours on TheForce.Net forums debating whether Han Solo's speed bonus stacked with the Millennium Falcon's hyperdrive rating, or wrote RPG campaigns set in the Outer Rim on Jedi Council Forums — this project is for you.

If you modded Star Wars Galaxies until the servers shut down, built lightsaber combat configs for Jedi Knight: Academy, ran Rebellion tournaments on mIRC, or still have your GOG copy installed just to hear that Imperial March fanfare one more time — this project is *especially* for you.
...
Open Rebellion reads the original game data files (you supply your own copy), converts them to clean JSON, and reimplements the simulation from the ground up in Rust. It runs natively on macOS and in the browser via WebAssembly.
