...
pub(crate) fn apply(
    ...
    let make_fleet = |is_alliance: bool,
        ...
        let expanded = (request.group_presentation || request.effect_presentation)
            && is_alliance == player_is_alliance;
        let ship_count = if request.effect_presentation && expanded {
            6
        } else if expanded {
            8
        } else {
