...
| Battle meshes and textures | Original `TACTICAL.DLL` contains 87 type-301 meshes and 397 type-303 texture/palette resources | P54 opt-in raw staging is implemented; binary X/type-303 decoding, runtime packing, and full semantic mapping remain open |
...
The original ordinal table is visible in [`FUN_005ab650`](../../../ghidra/notes/FUN_005ab650.c). Its candidate DAT names in the JSON are labeled separately because the original vtable `+0x24` implementors have not yet been joined to `CAPSHPSD`/`FIGHTSD`. The same function selects Death Star bases `5010`/`5020`. `FUN_00596ad0`, `FUN_005c2e60`, and `FUN_0059a850` establish the tactical planet arithmetic, but not its DAT input. P54 supplies reproducible, content-addressed raw extraction. Type-301 and type-303 resources still need deterministic decoding, visual classification, runtime transport, and original-runtime confirmation before the current primitive renderer can be replaced.
