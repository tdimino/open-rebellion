...
| Battle meshes and textures | Original `TACTICAL.DLL` contains 87 type-301 meshes and 397 type-303 texture/palette resources | P54 raw staging and P55 deterministic decoding are implemented. P56 proves the first exact pair. P57A packages `2560`–`2562`, binds `SDESTI52.BMP` and `SDESTI_M.BMP`, and proves source LOD selection. P57B1 proves the original cached-slot pattern in a live no-reload browser journey. Original view acceptance, production integration, and semantic mapping remain open |
