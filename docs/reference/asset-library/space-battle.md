...
| Battle audio                             | Original `TACTICAL.DLL` contains 66 WAVs; events `0x0d–0x14` map to 22 weapon variants at `13033–13054`. VOICEFXA/VOICEFXE supply 285 source-addressed tactical voices: 153 Alliance and 132 Imperial | P58-B16 proves exact weapon extraction and dispatch. P58-B17 proves faction battle-ready and task-force/RGBY command dispatch. P58-B18 proves complete 285-resource extraction, mapping, runtime-pack transport, muted browser routing, and selected withdrawal/result/Death Star/trench-run transition dispatch. P58-B19 proves mixed-task-force hostile-target rejection and exact faction feedback. P58-B20 proves final fighter-recovery, no-carrier-capacity, and disabled-hyperdrive withdrawal feedback. Remaining completion/destruction/warning/ordered-trench callers, exact shared-RNG sequencing, mixing, interruption, audible native comparison, and A0 acceptance remain open |
...
tables provided candidate tactical voice labels, including Death Star and
Alliance trench-run ranges. P58-B18 independently verifies every resource
against the owned executable's event arithmetic, owned DLL extraction, and the
local dual-engine audition manifest. The resulting compact authoritative map
is [`tactical-voice-map.json`](tactical-voice-map.json); external labels remain
research leads rather than copied authority.
