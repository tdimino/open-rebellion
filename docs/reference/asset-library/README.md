...
| [Space-battle graphics](space-battle.md) and [native tactical lookup](tactical-lookup.json) | Tactical BMP groups, exact native ordinal-to-mesh/texture bases, first source-proven three-LOD rule and cached-slot cycle, candidate ship/fighter identities, and remaining effects | 29 ship and eight fighter ordinal mappings; first family, LOD, camera, placement, palette, light, and device/material state proven; production DAT identity join remains open |
