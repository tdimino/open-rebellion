...
| First original tactical shell and control states; all original 3D assets decoded and transported; exact DAT resource joins; live capital families, fighter detail resources, system-selected planets, effects, fields, selected-capital damage, subsystem condition, exact field-source state, live subsystem damage, repair, maneuver state, and physical integration use recovered source contracts; other battle contents remain replacements | `crates/rebellion-render/src/tactical_view.rs`, `crates/rebellion-render/src/tactical_resources.rs`, `crates/rebellion-render/src/tactical_assets.rs`, `crates/rebellion-render/src/bmp_cache.rs`, `tools/stage-ui-assets` | Manual pp. 139–150, original TACTICAL resources, [P52 through P58F8 evidence](evidence/README.md), and [P58F8 evidence](evidence/2026-09-16-tactical-maneuver-movement.md) |
...
four tactical lanes. P57B2B3 binds stable DAT and original fleet-roster
identity to production participants, places them in the recovered source slots,
and targets the selected source-world point. P57B2C1 removes inferred mesh
...
gravity priority. P58F4 restores selected panel 1302, source-ordinal capital
portraits, matte transparency, and live faction-correct shield and hull meters.
P58F5 restores the five source-quantized subsystem-condition families and
retains exact tractor and gravity source IDs with the original source-side
capacity limits. P58F6 routes capital and fighter fire through the recovered
...
the stationary constructor state. Interactive command delivery, turning,
collision and formation behavior, power allocation, special objects, and A0
...
| UIP-F-009 | P1 | The original 640×480 tactical shell, first bitmap controls, full asset decode and transport, exact ship/fighter DAT joins, production capital/fighter/planet rendering, group controls, effects, fields, selected-capital damage, subsystem condition, exact field-source state, live subsystem damage, repair, maneuver state, and physical integration pass implementation gates. Original view acceptance, automatic grouping, exact planet placement, exact global RNG sequencing, interactive commands, turning, collision and formation behavior, power allocation, and incomplete result, special-state, and audio paths remain. | fail |
...
without committing proprietary output. P55 decodes it into a deterministic,
verified runtime store and checks all 87 meshes against Assimp 6.0.5 raw
import. P56 visibly renders one exact pair in a muted proof/control browser
matrix. P57A proves the complete related three-LOD family and the original
high- and reduced-detail selection predicate. Its
[evidence](evidence/2026-09-13-tactical-3d-lod-family.md) keeps original view
acceptance, production fleet binding, and every `TAC-*` cell open.
P57B2A through P57B2B3 recover the faction camera, bitmap D-pad, selected-object
target route, active-force extent, four tactical lanes, exact X-slot sequence,
stable DAT and fleet-roster identity, and selected source-world target. See the
[participant evidence](evidence/2026-09-13-tactical-participant-placement.md).
P57B2C1 adds the source-authored mesh transform and system-selected palette;
see its [evidence](evidence/2026-09-13-tactical-transform-palette.md). P57B2C2A
adds the source directional and ambient light rig; see its
...
through P58F8 restore system-selected planets, effects, fields,
...
and position integration; see the
