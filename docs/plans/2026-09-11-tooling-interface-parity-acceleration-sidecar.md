...
created: 2026-09-11
updated: 2026-09-12
tags: [interface, parity, browser, fixtures, screenshots, automation]
status: active
...
overlay, selection, pan, zoom, and both faction variants together. An
independent browser reviewer checks the complete generated evidence packet once
at the bundle gate.
...
flowchart LR
    ...
    E --> F[Independent evidence review and selected journeys]
...
when its reviewed baseline exists, and one JSON metric record locally. The
independent browser reviewer receives contact sheets plus direct access to the
lossless images, not recompressed video frames.
...
## ACC-05: Evidence-first independent review

The harness performs exhaustive deterministic probes. The independent browser
reviewer then:
...
The reviewer does not manually repeat every machine probe. Any unexplained mismatch,
...
| Bundle gate | Once per complete family | Full workspace tests, packaged production WASM, runtime pack and hashes, ledger and generated-doc checks, complete browser scenario matrix, visual diffs, independent browser review |
...
and independent browser review occur only at the bundle gate. A gate failure resumes focused
...
- number of full workspace, WASM, and independent browser-review runs;
- number of manually operated reviewer interactions;
...
at least an 80% reduction in manually operated reviewer probes, zero unmuted
...
4. Complete all `CMD-02` rendering and interaction work, then run one bundle
   gate and one independent browser review.
...
- an independent reviewer can decide readiness from the evidence packet plus selected journeys;
