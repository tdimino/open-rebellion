...
- [ ] M1: stop fleet redispatch/spawn/backlog feedback loops and prove deterministic native/WASM replay across five 5,000-tick seeds. F-007A prevents redispatch and travel resets. F-007B closes stale position, production attachment, and compatible arrival accumulation. F-007C closes ordinary player dispatch. F-007D closes the system combat backlog. The first F-007E checkpoint closes ownership, friendly-cycle, transit, troop-class, and repair-state defects; troop transport, conquest, faction balance, and victory remain before five-seed cross-runtime replay.
...
- Observation: F-007E removes the fleet-arena, transit, repair-start, and troop-class failures, then exposes missing conquest flow as the next campaign boundary.
  Evidence: five current runs finish with 4–6 fleets, none in transit, and 0–5 repair starts, but produce only 8–14 combined battles at three or four systems. Standard victory remains unreachable because fleets cannot carry troops into an occupation and principal-leader capture sequence.
